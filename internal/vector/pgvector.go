...
// PgVectorService 基于 PostgreSQL pgvector 插件的向量存储实现
// 核心特点：
// 1. 使用 PostgreSQL 原生向量类型（vector）
// 2. 支持 HNSW 索引加速搜索
// 3. 使用余弦距离算子（<=>）计算相似度
// 4. 事务保证数据一致性
...
// NewPgVectorService 创建 PgVector 服务
// 参数：
//   - db: *sql.DB 连接池（从 GORM 获取）
func NewPgVectorService(db *sql.DB) *PgVectorService {
	return &PgVectorService{db: db}
}
...
//
// SQL 说明：
// - embedding <=> $1::vector：计算余弦距离（越小越相似）
// - 1 - (embedding <=> $1::vector)：转换为相似度（越大越相似）
// - metadata->>'collection_name'：按集合过滤
func (s *PgVectorService) Search(ctx context.Context, collection string, queryVec []float32, topK int) ([]SearchResult, error) {
	...
	// 将 Go 的 []float32 转换为 PostgreSQL 向量格式 "[0.1,0.2,...]"
...
// 工作流程：
// 1. 开启事务
// 2. 按 pgvectorInsertBatchSize 分批，每批一条多行 INSERT
// 3. 提交事务
//
// 数据映射：
// - id → id（主键）
// - content → content（文本内容）
// - metadata → metadata（JSONB 格式）
// - vector → embedding（vector 格式）
...
//
// 幂等性：
// - 如果 chunk ID 不存在 → INSERT
// - 如果 chunk ID 已存在 → UPDATE
...
func (s *PgVectorService) DeleteByChunkID(ctx context.Context, collection string, chunkID string) error {
	...
	if err != nil {
		return fmt.Errorf("delete by chunk_id: %w", err)
	}
	deleted, _ := result.RowsAffected()
...
func (s *PgVectorService) DeleteByChunkIDs(ctx context.Context, collection string, chunkIDs []string) error {
	...
	if err != nil {
		return fmt.Errorf("delete by chunk_ids: %w", err)
	}
	deleted, _ := result.RowsAffected()
...
func (s *PgVectorService) EnsureCollection(ctx context.Context, collection string, dimension int) error {
	...
	err := s.db.QueryRowContext(ctx,
		"SELECT COUNT(*) FROM pg_indexes WHERE indexname = 'idx_kv_embedding_hnsw'").Scan(&count)
	if err != nil {
...
// buildMetadata 构建元数据映射
// 将 collection、doc_id、chunk_index 添加到元数据中
