...
func OrderPipelineNodes(nodes []NodeConfig) ([]NodeConfig, error) {
	...
	globalVisited := make(map[string]struct{}, len(nodes))
	for _, start := range startNodes {
		localVisited := make(map[string]struct{}, len(nodes))
		current := start
		for {
